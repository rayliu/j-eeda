...
public class DeliveryController extends Controller {
		...
		if (deliveryOrder == null) {
			...
			if("warehouseNatureYes".equals(warehouseNature)){  //******************调拨
				if(deliveryOrder.get("delivery_id")==null){
					...
					.set("route_from", getPara("route_from"))
					.set("pricetype", getPara("chargeType"))
					.set("from_warehouse_id", gateInSelect)
					.set("cargo_nature", cargoNature)
					.set("warehouse_nature", "warehouseNatureNo")
					.set("receivingunit", receivingunit)
					.set("ref_no", sign_document_no)
    ...
    public void findTransferOrderItems(){
        ...
        String sql = "select t1.id as tid,t2.id transfer_id,w.id as wid,p.id as pid,pro.id as productId,"
        		...
        		+ " LEFT JOIN depart_order dor ON dor.id = dp.depart_id"
        		+ " WHERE"
        		+ " dor. STATUS IS NOT NULL"
        		+ " AND dor. STATUS = '已入库'"
        		+ " AND dt.order_id = t1.order_id"
        		+ " AND dp.order_id = t1.order_id"
        		+ " AND dt.order_item_id = t1.id"
        		+ " )"
        		+ " else (t1.amount)"
        		+ " end) amount,"
