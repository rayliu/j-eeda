    ...
    public void pickuplist() {
        ...
        if (orderNo == null && departNo == null && beginTime == null && endTime == null
            ...
            sql = "select dor.*,"
            		...
            		+ " round((select sum(ifnull(toi.volume, 0)) from transfer_order_item toi where toi.order_id in(select dt.order_id from	depart_transfer dt where dt.pickup_id = dor.id)),2) volume,"
            		+ " round((select sum(ifnull(toi.sum_weight, 0)) from transfer_order_item toi where toi.order_id in(select dt.order_id from	depart_transfer dt where dt.pickup_id = dor.id)),2) weight,"
    ...
    public void createList() {
        ...
        if (orderNo == null && status == null && address == null && customer == null && routeFrom == null && routeTo == null
            ...
            sqlTotal = "select count(1) total  from transfer_order tor " + " left join party p on tor.customer_id = p.id "
                    ...
                    + " where tor.operation_type =  'own' and tor.status not in ('已入库','已签收', '已收货') and ifnull(tor.pickup_assign_status, '') !='"
