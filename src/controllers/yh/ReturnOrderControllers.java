...
public class ReturnOrderControllers extends Controller {
    ...
    public void checkorder() {
        ...
        message = Db
                .find("select r.id,r.status_code,r.create_date,r.transaction_status,r.creator,"
                        ...
                        + id
                        + "')) counterman " + "from return_order r where  r.transfer_order_id='" + id + "'");
    ...
    public void itemlist() {
        ...
        // 获取货损条数和货品id
        totallist = Db.find("select count(1) total  , ITEM_ID  from TRANSFER_ORDER_ITEM_DETAIL where order_id ='" + id
                + "' and is_damage =true GROUP by item_id  ");
        // 货品信息
        if (nature.equals("ATM")) {
            if (totallist.size() > 0) {
                ...
                itemlist = Db.find("SELECT ITEM_NAME ,AMOUNT ,VOLUME ,WEIGHT ,REMARK  FROM TRANSFER_ORDER_ITEM where ORDER_ID ='" + id
                        + "'");
            ...
            itemlist = Db.find("SELECT * FROM TRANSFER_ORDER_ITEM where ORDER_ID ='" + id + "'");
        }

        for (int i = 0; i < itemlist.size(); i++) {
            try {
                if (itemlist.get(i).get("id") == totallist.get(i).get("item_id")
                        || itemlist.get(i).get("item_id") == totallist.get(i).get("item_id")) {
                    String amount = String.valueOf(itemlist.get(i).get("amount"));
                ...
                itemlist.get(i).set("lastamount", itemlist.get(i).get("amount"));
