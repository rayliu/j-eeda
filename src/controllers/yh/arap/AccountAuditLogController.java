    ...
    public void list() {
        ...
        if(!"-1".equals(ids)){
        	...
        	sql = "select aaal.*,aci.order_no invoice_order_no,ifnull(ul.c_name, ul.user_name) user_name, fa.bank_name,"
        			+ " (CASE WHEN aaal.source_order = '手工收入单' THEN (SELECT order_no FROM arap_charge_invoice WHERE id = aaal.misc_order_id)"
        			+ " WHEN aaal.source_order = '报销单' THEN(SELECT group_concat(DISTINCT order_no SEPARATOR '<br/>')FROM reimbursement_order WHERE id IN ( SELECT d.reimbursement_order_id FROM arap_cost_pay_confirm_order_detail d, arap_cost_pay_confirm_order_detail_log dl WHERE d.id = dl.detail_id AND dl.order_id = aaal.reimbursement_order_id))"
        			+ " WHEN aaal.source_order = '行车报销单' THEN(SELECT group_concat(DISTINCT order_no SEPARATOR '<br/>')FROM reimbursement_order WHERE id IN ( SELECT d.reimbursement_order_id FROM arap_cost_pay_confirm_order_detail d, arap_cost_pay_confirm_order_detail_log dl WHERE d.id = dl.detail_id AND dl.order_id = aaal.reimbursement_order_id))"
