    ...
    public void list() {
        ...
        if(!"-1".equals(ids)){
        	...
        	sql = "select aaal.*,aci.order_no invoice_order_no,ifnull(ul.c_name, ul.user_name) user_name, fa.bank_name,"
        			+ " (CASE WHEN aaal.source_order = '手工收入单' THEN (SELECT order_no FROM arap_misc_charge_order WHERE id = aaal.invoice_order_id)"
        			+ " WHEN aaal.source_order = '报销单' THEN(SELECT group_concat(DISTINCT order_no SEPARATOR '<br/>')FROM reimbursement_order WHERE id = aaal.invoice_order_id)"
        			+ " WHEN aaal.source_order = '行车报销单' THEN(SELECT group_concat(DISTINCT order_no SEPARATOR '<br/>')FROM reimbursement_order WHERE id = aaal.invoice_order_id)"
        			...
        			+ " WHEN aaal.source_order = '应收对账单' THEN(SELECT group_concat( DISTINCT order_no SEPARATOR '<br/>') FROM arap_charge_order WHERE id = aaal.invoice_order_id)"
        			+ " WHEN aaal.source_order = '应收开票记录单' THEN(SELECT group_concat( DISTINCT aci.order_no SEPARATOR '<br/>') FROM arap_charge_invoice aci WHERE aci.id = aaal.invoice_order_id)"
        			+ " WHEN aaal.source_order = '行车单' THEN(SELECT group_concat(DISTINCT order_no SEPARATOR '<br/>') FROM car_summary_order WHERE id = aaal.invoice_order_id)END) order_no"
