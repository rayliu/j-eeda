    ...
    public void list() {
        ...
        if(true){
        	sql = " select * from (select aaal.*,aci.order_no invoice_order_no,ifnull(ul.c_name, ul.user_name) user_name, fa.bank_name,"
				    ...
				    + " THEN ifnull(( SELECT group_concat( DISTINCT aci.order_no SEPARATOR '<br/>' ) FROM arap_cost_invoice_application_order aci LEFT JOIN arap_cost_pay_confirm_order_detail acp on acp.application_order_id = aci.id where acp.order_id = aaal.invoice_order_id )"
				    + " ,(select order_no from arap_cost_invoice_application_order where id = aaal.invoice_order_id))"
