...
@RequiresPermissions(value = {PermissionConstant.PERMSSION_CCOI_UPDATE})
	...
	public void costConfirmOrderList() {
    	...
    	String sqlTotal = "";
    	String sql = " select * from (select distinct dor.id, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name, tor.planning_time ,dor.order_no order_no,dor.status,c.abbr spname,toi.amount,ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dor.create_stamp create_stamp,ul.user_name creator,'配送' business_type, (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.type = '应付') pay_amount,"
							...
							+ " left join location lo on lo.code = dpr.route_from "
							+ " left join location lo2 on lo2.code = dpr.route_to "
							+ " left join office oe on oe.id = tor.office_id where (ifnull(dtr.depart_id, 0) > 0) and dpr.audit_status='已确认' group by dpr.id"
							+ " union "
							+ " select distinct dpr.id, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name ,tor.planning_time ,dpr.depart_no order_no,dpr.status,c.abbr spname,toi.amount,ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dpr.create_stamp create_stamp,ul.user_name creator,'提货' business_type, (select sum(amount) from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = dpr.id and fi.type = '应付') pay_amount, "
    	...
    	double changeAmount1=0.0;
 		for(int i=0;i<orderIdsArr.length;i++){
            if("提货".equals(orderNoArr[i])){
            	...
            	if("提货".equals(type)){for(int j=0;j<BillingOrders.size();j++){
            		Record b=BillingOrders.get(j);
            	...
            	rec = Db.findFirst("select sum(amount) amount,sum(change_amount) change_amount from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = ?", orderIdsArr[i]);
            	if(rec.getDouble("change_amount")!=null){
            		changeAmount = changeAmount + rec.getDouble("change_amount");
            	}
            	else{
