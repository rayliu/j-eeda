...
public class CostCheckOrderController extends Controller {
	...
	public void unSelectedList() {
		...
		String sql = " select * from (select distinct dor.id,dofi.id did,IFNULL(c2.address,IFNULL(toid.notify_party_company,'')) receivingunit, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name, tor.planning_time ,dor.order_no order_no,dor.status,"
				+ " dor.sp_id sp_id, c.abbr spname,c1.abbr customer_name,"
				...
				+ " dor.ref_no,ul2.c_name confirm_by ,dor.confirm_stamp "
				...
				+ " left join product prod on toi.product_id = prod.id "
				+ " left join user_login ul on ul.id = dor.create_by "
				+ " left join user_login ul2 on ul2.id = dor.confirm_by"
				+ " left join warehouse w on w.id = dor.from_warehouse_id "
				...
				+ " ,ul2.c_name confirm_by ,dpr.confirm_stamp "
				...
				+ " left join product prod on toi.product_id = prod.id "
				+ " left join user_login ul on ul.id = dpr.create_by"
				+ " left join user_login ul2 on ul2.id = dpr.confirm_by"
				+ " left join party p on p.id = dpr.sp_id left join contact c on c.id = p.contact_id "
				...
				+ " tor.sp_id sp_id, con.abbr spname,c_c.abbr customer_name,sum(toi.amount) amount,round(sum(ifnull(prod.volume,toi.volume)),2) volume,round(sum(ifnull(prod.weight,toi.weight)),2) weight,ior.create_stamp create_stamp,ul.user_name creator,"
				+ " '保险' business_type, "
				...
				+ " ,ul2.c_name confirm_by ,ior.confirm_stamp "
				...
				+ " left join user_login ul2 on ul2.id = ior.confirm_by"
				+ " left join office oe on oe.id = tor.office_id "
			    ...
			    + " and '"+is_delivery+"' = 'N' "
                + (sp_id2.equals("7151")?" and ior.insurance_id = '2432'":" and ior.insurance_id ="+sp_id2)
                + " group by ior.id"
				...
				+ " amco.sp_id sp_id, c.abbr spname, c1.abbr customer_name, NULL AS amount,"
				...
				+ " ,ul2.c_name confirm_by ,amco.confirm_stamp "
				...
				+ " LEFT JOIN user_login ul2 ON ul2.id = amco.confirm_by"
