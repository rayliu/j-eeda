...
@RequiresPermissions(value = {PermissionConstant.PERMSSION_CCOI_UPDATE})
	...
	public void unSelectedList() {
    	...
    	String sql = " select * from (select distinct dor.id,dofi.id did,IFNULL(c2.address,IFNULL(toid.notify_party_company,'')) receivingunit, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name, tor.planning_time ,dor.order_no order_no,dor.status,"
    			+ " c.id sp_id, c.abbr spname,c1.abbr customer_name,"
    						...
    						+ " (SELECT sum(change_amount) FROM delivery_order_fin_item dofi LEFT JOIN fin_item fi ON fi.id = dofi.fin_item_id"
							...
							+ " '零担' business_type, "
							+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.depart_order_id = dpr.id and fi.type = '应付') pay_amount,"
							+ " (SELECT sum(change_amount) FROM depart_order_fin_item dofi LEFT JOIN fin_item fi ON fi.id = dofi.fin_item_id WHERE dofi.depart_order_id = dpr.id AND fi.type = '应付') change_amount,"
							...
							+ " '' serial_no,tor.customer_order_no customer_order_no, '' ref_no"
							...
							+ " (select sum(change_amount) from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = dpr.id and fi.type = '应付') change_amount,"
							...
							+ " group_concat(distinct tor.order_no separator '<br/>') transfer_order_no,ior.sign_status return_order_collection,ior.remark,dpr.booking_note_number,oe.office_name office_name, "
							+ " '' serial_no, tor.customer_order_no customer_order_no,'' ref_no"
							...
							+ " (SELECT sum(change_amount) FROM arap_misc_cost_order_item amcoi LEFT JOIN fin_item fi ON fi.id = amcoi.fin_item_id WHERE amcoi.misc_order_id = amco.id AND fi.type = '应付') change_amount,"
	...
	public void costConfirmListById() {
    	...
    	String searchSql = "select distinct dor.id,dofi.id did,dor.order_no order_no,dor.status,c.abbr spname,c1.abbr customer_name,IFNULL(c2.address,IFNULL(toid.notify_party_company,'')) receivingunit,(select count(*) from delivery_order_item doit where doit.delivery_id = dor.id) amount,ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dor.create_stamp create_stamp,ul.user_name creator,'配送' business_type,"
    						...
    						+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.type = '应付') pay_amount,(SELECT ifnull(sum(change_amount),0) FROM delivery_order_fin_item dofi WHERE dofi.order_id = dor.id) change_amount, group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = doi.transfer_order_id group by tor.id) separator '<br/>') transfer_order_no,dor.sign_status return_order_collection,dor.remark,oe.office_name office_name "
							+ " from delivery_order dor"
							+ " left join party p on p.id = dor.sp_id "
							+ " LEFT JOIN delivery_order_fin_item dofi on dofi.order_id=dor.id"
							+ " left join contact c on c.id = p.contact_id "
							...
							+ " WHERE dpr2.id = dpr.id )END) amount,"
							+ " ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dpr.create_stamp create_stamp,ul.user_name creator,'零担' business_type, "
							...
							+ " union "
							+ " select distinct dpr.id,dofi.id did,dpr.depart_no order_no,dpr.status,c.abbr spname,c1.abbr customer_name,(CASE tor.arrival_mode WHEN 'gateIn' THEN w.warehouse_name WHEN 'delivery' THEN tor.receiving_address WHEN 'deliveryToFactory' THEN tor.receiving_address WHEN 'deliveryToWarehouse' OR 'deliveryToFachtoryFromWarehouse' THEN w.warehouse_name ELSE tor.receiving_address END) receivingunit,"
							+ " toi.amount,ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dpr.create_stamp create_stamp,ul.user_name creator,'提货' business_type, "
							+ " ifnull((SELECT NAME FROM location WHERE CODE = dpr.route_from),'') route_from,ifnull((SELECT NAME FROM location WHERE CODE = dpr.route_to),'') route_to,"
							+ " (select sum(amount) from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = dpr.id and fi.type = '应付') pay_amount,(SELECT ifnull(sum(change_amount),0) FROM pickup_order_fin_item dofi WHERE dofi.pickup_order_id = dpr.id) change_amount, group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = dtr.order_id) separator '<br/>') transfer_order_no,dpr.sign_status return_order_collection,dpr.remark,oe.office_name office_name "
							+ " from depart_order dpr "
							+ " left join depart_transfer dtr on dtr.pickup_id = dpr.id"
							+ " LEFT JOIN pickup_order_fin_item dofi on dofi.pickup_order_id=dpr.id"
							+ " left join transfer_order tor on tor.id = dtr.order_id "
							...
							+ " union "
							+ " select distinct ior.id,ifi.id did,ior.order_no order_no,ior.status,'保险公司' spname,c1.abbr customer_name,'' receivingunit,sum(toi.amount) amount,sum(ifnull(prod.volume,toi.volume)) volume,sum(ifnull(prod.weight,toi.weight)) weight,ior.create_stamp create_stamp,ul.user_name creator,'保险' business_type, '' route_from ,'' route_to,"
							+ " (select sum(insurance_amount) from insurance_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.insurance_order_id = ior.id and fi.type = '应付') pay_amount,(SELECT ifnull(sum(change_amount),0) FROM insurance_fin_item dofi WHERE dofi.insurance_order_id = ior.id) change_amount, group_concat(distinct tor.order_no separator '<br/>') transfer_order_no,ior.sign_status return_order_collection,ior.remark,oe.office_name office_name "
							+ " from insurance_order ior "
							+ " left join transfer_order tor on ior.id = tor.insurance_id "
							+ " LEFT JOIN insurance_fin_item ifi ON ifi.insurance_order_id=ior.id"
							+ " left join transfer_order_item toi on toi.order_id = tor.id "
							...
							+ " union "
							+ " SELECT DISTINCT amco.id,amcoi.id did,amco.order_no,amco. STATUS,c.abbr spname,c1.abbr customer_name,'' receivingunit,amco.total_amount,NULL AS volume,NULL AS weight,amco.create_stamp,ul.user_name creator,'成本单' business_type,'' route_from ,'' route_to,"
							+ " (SELECT sum(amount) FROM arap_misc_cost_order_item amcoi LEFT JOIN fin_item fi ON fi.id = amcoi.fin_item_id WHERE amcoi.misc_order_id= amco.id ) pay_amount,(SELECT ifnull(sum(change_amount),0) FROM arap_misc_cost_order_item amcoi LEFT JOIN fin_item fi ON fi.id = amcoi.fin_item_id WHERE amcoi.misc_order_id= amco.id ) AS change_amount,NULL AS transfer_order_no,NULL AS return_order_collection,amco.remark remark,NULL AS office_name "
							+ " FROM arap_misc_cost_order amco "
							+ " LEFT JOIN arap_misc_cost_order_item amcoi ON amcoi.misc_order_id = amco.id "
							+ " LEFT JOIN user_login ul ON ul.id = amco.create_by "
							+ " LEFT JOIN party p ON amco.sp_id = p.id "
	...
	public void updateDepartOrderFinItem() {
    	...
    	if("提货".equals(type)){
    		rec3 = Db.findFirst("select * from pickup_order_fin_item  where pickup_order_id =?", tId);
        	if(rec3==null){
        		PickupOrderFinItem dofi = new PickupOrderFinItem();
        		dofi.set("pickup_order_id", tId);
        		dofi.set("fin_item_id", "1");
        		dofi.set("amount", "0");
        		dofi.set("change_amount", value);
        		dofi.set("STATUS", "新建");
        		dofi.set("create_date", new Date());
        		dofi.set("remark", "对账确认金额为0增加明细");
        		dofi.save();
        	}
            	rec1 = Db.findFirst("select sum(amount) sum_amount from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = ? and fi.type = '应付'", tId);
            	totalAmount = totalAmount + rec1.getDouble("sum_amount");	
            	List<Record> BillingOrders = Db.find("select id,amount from pickup_order_fin_item where fin_item_id!=7 and pickup_order_id=?",tId);
            		...
            		if(changeAmount1==0.0){
            			pickuporderfinitem.set("change_amount", value);
                		pickuporderfinitem.update();
            		}
            		else{
            			pickuporderfinitem.set("change_amount", num);
            	...
            	rec = Db.findFirst("select sum(amount) amount,sum(change_amount) change_amount from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where fin_item_id!=7 and  dofi.pickup_order_id = ?", tId);
            	if(rec.getDouble("change_amount")!=null){
            		changeAmount = changeAmount + rec.getDouble("change_amount");
            	}
            	else{
            	...
            	rec3 = Db.findFirst("select * from depart_order_fin_item  where depart_order_id =?", tId);
            	if(rec3==null){
            	...
            	rec1 = Db.findFirst("select sum(amount) sum_amount from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.depart_order_id = ? and fi.type = '应付'", tId);
            	totalAmount = totalAmount + rec1.getDouble("sum_amount");
            	List<Record> BillingOrders = Db.find("select id,amount from depart_order_fin_item where depart_order_id=?",tId);
                	...
                	rec = Db.findFirst("select sum(amount) amount, sum(change_amount) change_amount from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.depart_order_id = ?", tId);
                	if(rec.getDouble("change_amount")!=null){
                		changeAmount = changeAmount + rec.getDouble("change_amount");
                	}
                	else{
            	...
            	rec3 = Db.findFirst("select * from delivery_order_fin_item  where order_id =?", tId);
            	if(rec3==null){
            	...
            	rec1 = Db.findFirst("select sum(amount) sum_amount from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = ? and fi.type = '应付'", tId);
            	totalAmount = totalAmount + rec1.getDouble("sum_amount");
            	...
            	rec = Db.findFirst("select sum(amount) amount, sum(change_amount) change_amount from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = ?", tId);
            	if(rec.getDouble("change_amount")!=null){
            		changeAmount = changeAmount + rec.getDouble("change_amount");
            	}
            	else{
            	...
            	rec1 = Db.findFirst("select sum(amount) sum_amount from arap_misc_cost_order_item amcoi left join fin_item fi on fi.id = amcoi.fin_item_id  where amcoi.misc_order_id = ? and fi.type ='应付'",tId);
            	totalAmount = totalAmount + rec1.getDouble("sum_amount");
            	...
            	rec = Db.findFirst("select sum(change_amount) change_amount from arap_misc_cost_order_item amcoi left join fin_item fi on fi.id = amcoi.fin_item_id  where amcoi.misc_order_id = ?",tId);
            	if(rec.getDouble("change_amount")!=null){
            	changeAmount = changeAmount + rec.getDouble("change_amount");
            	}
            	...
            	insurancefinitem.update();
            	rec1 = Db.findFirst("select sum(insurance_amount) sum_amount from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id  where ifi.insurance_order_id = ? and fi.type ='应付'",tId);
            	totalAmount = totalAmount + rec1.getDouble("sum_amount");
            	rec = Db.findFirst("select sum(insurance_amount) amount,sum(change_amount) change_amount from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id  where ifi.insurance_order_id = ?",tId);
            	if(rec.getDouble("change_amount")!=null){
            		changeAmount = changeAmount + rec.getDouble("change_amount");
            	}
            	else{
