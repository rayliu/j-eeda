...
public class CostCheckOrderController extends Controller {
	...
	public void edit() {
		...
		for (int i = 0; i < orderIdsArr.length; i++) {
			if ("提货".equals(orderNoArr[i])) {
				...
				totalamount = totalamount + rec1.getDouble("sum_amount");
				rec = Db.findFirst(
						"select sum(amount) change_amount from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id  where ifi.insurance_order_id = ?",
						orderIdsArr[i]);
				if (rec.getDouble("change_amount") != null) {
					changeamount = changeamount
							+ rec.getDouble("change_amount");
	...
	public void costConfirmListById() {
		...
		String searchSql = "select distinct dor.id,dofi.id did,dor.order_no order_no,dor.status,c.abbr spname,c1.abbr customer_name,IFNULL(c2.address,IFNULL(toid.notify_party_company,'')) receivingunit,(select count(*) from delivery_order_item doit where doit.delivery_id = dor.id) amount,ifnull(prod.volume,toi.volume) volume,ifnull(prod.weight,toi.weight) weight,dor.create_stamp create_stamp,ul.user_name creator,"
				...
				+ " (select ifnull(sum(amount),0) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.type = '应付' and IFNULL(dofi.cost_source,'') != '对账调整金额') pay_amount,(SELECT ROUND(ifnull(sum(amount),0),2) FROM delivery_order_fin_item dofi WHERE dofi.order_id = dor.id) change_amount, group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = doi.transfer_order_id group by tor.id) separator '<br/>') transfer_order_no,"
				+ " group_concat(DISTINCT (SELECT group_concat(toid.serial_no SEPARATOR '') FROM	transfer_order_item_detail toid	WHERE	toid.delivery_id = dor.id) SEPARATOR '<br/>')serial_no,dor.sign_status return_order_collection,dor.remark,oe.office_name office_name "
				+ " from delivery_order dor"
				+ " left join party p on p.id = dor.sp_id "
				+ " LEFT JOIN delivery_order_fin_item dofi on dofi.order_id=dor.id"
				+ " left join contact c on c.id = p.contact_id "
				...
				+ " (select ifnull(sum(amount),0) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.depart_order_id = dpr.id and fi.type = '应付' and IFNULL(dofi.cost_source,'') != '对账调整金额') pay_amount,(SELECT ROUND(ifnull(sum(amount),0),2) FROM depart_order_fin_item dofi WHERE dofi.depart_order_id = dpr.id) change_amount, group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = dtr.order_id) separator '<br/>') transfer_order_no,"
				+ " group_concat(DISTINCT (SELECT group_concat(toid.serial_no SEPARATOR '') FROM transfer_order_item_detail toid	WHERE toid.depart_id = dtr.id) SEPARATOR '<br/>')serial_no,dpr.sign_status return_order_collection,dpr.remark,oe.office_name office_name "
				+ " from depart_order dpr "
				+ " left join depart_transfer dtr on dtr.depart_id = dpr.id"
				+ " LEFT JOIN depart_order_fin_item dofi on dofi.depart_order_id=dpr.id"
				+ " left join transfer_order tor on tor.id = dtr.order_id "
				...
				+ " ifnull((SELECT NAME FROM location WHERE CODE = dpr.route_from),'') route_from,ifnull((SELECT NAME FROM location WHERE CODE = dpr.route_to),'') route_to,"
				+ " (select ifnull(sum(amount),0) from pickup_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.pickup_order_id = dpr.id and fi.type = '应付' and dofi.fin_item_id!=7 and IFNULL(dofi.cost_source,'') != '对账调整金额') pay_amount,(SELECT ROUND(ifnull(sum(amount),0),2) FROM pickup_order_fin_item dofi WHERE dofi.pickup_order_id = dpr.id and dofi.fin_item_id!=7) change_amount, group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = dtr.order_id) separator '<br/>') transfer_order_no,"
				+ " group_concat(DISTINCT (SELECT	group_concat(toid.serial_no SEPARATOR '') FROM	transfer_order_item_detail toid	WHERE toid.pickup_id = dtr.id) SEPARATOR '<br/>')serial_no,dpr.sign_status return_order_collection,dpr.remark,oe.office_name office_name "
				+ " from depart_order dpr "
				+ " left join depart_transfer dtr on dtr.pickup_id = dpr.id"
				+ " LEFT JOIN pickup_order_fin_item dofi on dofi.pickup_order_id=dpr.id"
				+ " left join transfer_order tor on tor.id = dtr.order_id "
				...
				+ " (select ifnull(sum(insurance_amount),0) from insurance_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.insurance_order_id = ior.id and fi.type = '应付' and IFNULL(dofi.cost_source,'') != '对账调整金额') pay_amount,(SELECT ROUND(ifnull(sum(insurance_amount),0),2) FROM insurance_fin_item dofi WHERE dofi.insurance_order_id = ior.id) change_amount, group_concat(distinct tor.order_no separator '<br/>') transfer_order_no,"
				+ " group_concat(DISTINCT toid.serial_no SEPARATOR '<br/>')serial_no,ior.sign_status return_order_collection,ior.remark,oe.office_name office_name "
				+ " from insurance_order ior "
				+ " left join transfer_order tor on ior.id = tor.insurance_id "
				+ " LEFT JOIN insurance_fin_item ifi ON ifi.insurance_order_id=ior.id"
				+ " left join transfer_order_item toi on toi.order_id = tor.id "
				...
				+ " (SELECT ifnull(sum(amount),0) FROM arap_misc_cost_order_item amcoi LEFT JOIN fin_item fi ON fi.id = amcoi.fin_item_id WHERE amcoi.misc_order_id= amco.id ) pay_amount,(SELECT ROUND(ifnull(sum(amount),0),2) FROM arap_misc_cost_order_item amcoi LEFT JOIN fin_item fi ON fi.id = amcoi.fin_item_id WHERE amcoi.misc_order_id= amco.id ) AS change_amount,NULL AS transfer_order_no,NULL AS serial_no,NULL AS return_order_collection,amco.remark remark,NULL AS office_name "
				+ " FROM arap_misc_cost_order amco "
				+ " LEFT JOIN arap_misc_cost_order_item amcoi ON amcoi.misc_order_id = amco.id "
				+ " LEFT JOIN user_login ul ON ul.id = amco.create_by "
				+ " LEFT JOIN party p ON amco.sp_id = p.id "
