...
@RequiresPermissions(value = {PermissionConstant.PERMSSION_CCOI_UPDATE})
	...
	public void unSelectedList() {
    	...
    	String sql = " select * from (select distinct dor.id,dofi.id did,IFNULL(c2.address,IFNULL(toid.notify_party_company,'')) receivingunit, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name, tor.planning_time ,dor.order_no order_no,dor.status,c.abbr spname,"
							...
							+ " union"
							+ " select distinct dpr.id,dofi.id did,(CASE tor.arrival_mode WHEN 'gateIn' THEN w.warehouse_name WHEN 'delivery' THEN c2.address WHEN 'deliveryToFactory' THEN c2.address "
							+ " WHEN 'deliveryToWarehouse' OR 'deliveryToFachtoryFromWarehouse' THEN w.warehouse_name ELSE c2.address END)  receivingunit, dpr.route_from ,lo.name from_name ,dpr.route_to ,lo2.name to_name ,tor.planning_time ,dpr.depart_no order_no,dpr.status,c.abbr spname,"
							...
							+ " LEFT JOIN party p2 ON p2.id = tor.notify_party_id"
							+ " LEFT JOIN contact c2 ON c2.id = p2.contact_id"
