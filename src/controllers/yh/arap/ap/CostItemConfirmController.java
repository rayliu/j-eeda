    ...
    public void list() {
       ...
       /* String sql = "select * from (select distinct dor.id,"
        ...
        String sql = "select * from (select distinct dor.id,"
				...
				+ "ul.user_name creator,"
				+ "'保险' business_type, "
				+ "(select sum(insurance_amount) from insurance_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.insurance_order_id = ior.id and fi.type = '应付') pay_amount, "
