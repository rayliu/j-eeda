    ...
    public void list() {
       ...
       /* String sql = "select * from (select distinct dor.id,"
        ...
        String sql = "select * from (select distinct dor.id,"
				...
				+ " (select sum(ifnull(prod.volume,toi.volume)*amount) from transfer_order_item toi left join product prod on toi.product_id = prod.id  where toi.order_id in(select tro.id from transfer_order tro where tro.insurance_id  = ior.id )) volume,"
				+ " (select sum(ifnull(prod.weight,toi.weight)*amount) from transfer_order_item toi left join product prod on toi.product_id = prod.id  where toi.order_id in(select tro.id from transfer_order tro where tro.insurance_id  = ior.id )) weight,"
				+ " ior.create_stamp create_stamp,"
				+ " ul.user_name creator,"
				+ " '保险' business_type, "
