    ...
    public void list() {
       ...
       /* String sql = "select * from (select distinct dor.id,"
        ...
        String sql = "select * from (select distinct dor.id,"
        		...
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='YSF') as transport_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='BYF') as carry_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='SLF') as climb_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='BXF') as insurance_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='YFTHF') as take_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='YFAZF') as anzhuang_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='CCF') as cangchu_cost,"
        		+ " (select sum(amount) from delivery_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where dofi.order_id = dor.id and fi.code='YFOF') as other_cost"
				+ " from delivery_order dor"
				+ " left join party p on p.id = dor.sp_id "
				+ " left join contact c on c.id = p.contact_id "
				+ " left join delivery_order_item doi on doi.delivery_id = dor.id "
				...
				+ "dpr.remark,"
				+ "oe.office_name office_name,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='YSF') transport_cost, "
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='BYF') carry_cost,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='SLF') climb_cost,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='BXF') insurance_cost,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='YFTHF') take_cost, "
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='YFAZF') anzhuang_cost,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='CCF') cangchu_cost,"
				+ " (select sum(amount) from depart_order_fin_item dofi left join fin_item fi on fi.id = dofi.fin_item_id where depart_order_id = dpr.id and fi.code='YFOF') other_cost"
				+ " from depart_order dpr "
				+ " left join depart_transfer dtr on dtr.depart_id = dpr.id"
				+ " left join transfer_order tor on tor.id = dtr.order_id "
				+ " left join transfer_order_item toi on toi.order_id = tor.id "
				...
				+ " dpr.remark,"
				+ " oe.office_name office_name,"
				+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='YSF') as transport_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='BYF') as carry_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='SLF') as climb_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='BXF') as insurance_cost,"
				+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='YFTHF') as take_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='YFAZF') as anzhuang_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='CCF') as cangchu_cost,"
        		+ " (select sum(amount) from pickup_order_fin_item pofi left join fin_item fi on fi.id = pofi.fin_item_id where pofi.pickup_order_id = dpr.id and fi.code='YFOF') as other_cost"
				+ " from depart_order dpr "
				+ " left join depart_transfer dtr on dtr.pickup_id = dpr.id"
				+ " left join transfer_order tor on tor.id = dtr.order_id "
				+ " left join transfer_order_item toi on toi.order_id = tor.id "
				...
				+ " ior.status,"
				+ " '保险公司' spname,"
				+ " (select sum(amount) from transfer_order_item toi where toi.order_id in(select tro.id from transfer_order tro where tro.insurance_id  = ior.id )) amount,"
				...
				+ " ul.user_name creator,"
				+ " '保险' business_type, "
				+ " (select sum(insurance_amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.type = '应付') pay_amount, "
				+ " group_concat(distinct tor.order_no separator '\r\n') transfer_order_no,"
				+ " ior.sign_status return_order_collection,"
				+ " ior.remark,"
				+ " oe.office_name office_name,"
				+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='YSF') as transport_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='BYF') as carry_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='SLF') as climb_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='BXF') as insurance_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='YFTHF') as take_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='YFAZF') as anzhuang_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='CCF') as cangchu_cost,"
        		+ " (select sum(amount) from insurance_fin_item ifi left join fin_item fi on fi.id = ifi.fin_item_id where ifi.insurance_order_id = ior.id and fi.code='YFOF') as other_cost"
				+ " from insurance_order ior "
				+ " left join transfer_order tor on ior.id = tor.insurance_id "
				+ " left join transfer_order_item toi on toi.order_id = tor.id "
				+ " left join user_login ul on ul.id = ior.create_by"
