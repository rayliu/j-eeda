...
public class CostItemConfirmController extends Controller {
    ...
    public void list() {
        ...
        String sql = "select cast(planning_time as CHAR) planning_time1, A.* from (select distinct dor.id,"
        		...
        		//+ " (SELECT CASE WHEN tor.cargo_nature = 'ATM' THEN(SELECT sum(doi1.amount) FROM delivery_order_item doi1 WHERE doi1.delivery_id = dor.id)"
                //+ " WHEN tor.cargo_nature = 'cargo' THEN(SELECT sum(doi2.amount) FROM delivery_order_item doi2 WHERE doi2.delivery_id = dor.id)END)amount,"
        		...
        		+ " ul.user_name creator,tor.customer_order_no customer_order_no,"
        		+ " dor.ref_no ref_no,"
        		+ " '配送' business_type, "
...
//				+ " left join product prod on toi.product_id = prod.id "
				...
				+ " DATE(dpr.create_stamp) create_stamp,ul.user_name creator,tor.customer_order_no customer_order_no,"
				+ " null as ref_no,"
				+ " '提货' business_type, "
				+ " null as booking_note_number,"
				+ " null as customer_delivery_no,"
...
//				+ " (select round(sum(ifnull(prod.weight,toi.weight)*amount),2) from transfer_order_item toi left join product prod on toi.product_id = prod.id  where toi.order_id in(select tro.id from transfer_order tro where tro.insurance_id  = ior.id )) weight,"
				...
				+ " amco.create_stamp,ul.user_name creator,'' customer_order_no,null as ref_no,'成本单' business_type,"
