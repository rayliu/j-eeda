    ...
    public void list() {
       ...
       /* String sql = "select * from (select distinct dor.id,"
        ...
        String sql = "select * from (select distinct dor.id,"
				...
				+ " '提货' business_type, "
				+ " null as booking_note_number,"
				+ " (select sum(pofi.amount) from pickup_order_fin_item pofi  where pofi.pickup_order_id = dpr.id and pofi.fin_item_id !='7') pay_amount, "
				+ " group_concat(distinct (select tor.order_no from transfer_order tor where tor.id = dtr.order_id) separator '\r\n') transfer_order_no,"
				+ " dpr.sign_status return_order_collection,"
				+ " dpr.remark,"
				...
				+ " where unix_timestamp(tom.create_stamp) > unix_timestamp('2015-06-01 10:34:36') and (ifnull(dtr.pickup_id, 0) > 0) and dpr.audit_status='新建' AND toid.item_id = toi.id and pofi.id is not null and dpr.combine_type='PICKUP' and tom.status ='已入货场' and tom.type='PICKUPORDERMILESTONE' group by dpr.id"
				+ " union "
				+ " select distinct ior.id,"
				+ " ior.order_no order_no,"
				+ " ior.status,"
