...
public class ChargeCheckOrderController extends Controller {
	...
	private List<Record> getItemLIst(String returnOrderIds, String miscOrderIds) {
		String itemReturnSql = "select distinct ror.*, "
				...
				+ " (SELECT IFNULL(lo4. NAME,lo3. NAME) FROM location l LEFT JOIN location lo3 ON lo3.`code` = l.pcode LEFT JOIN location lo4 ON lo4.`code` = lo3.pcode WHERE l. CODE = tor.route_to) province,"
			...
			String itemMiscSql=" (SELECT amco.id id,amco.order_no order_no,NULL status_code,amco.create_stamp create_date,"
				...
				+ " null province, null receipt_address, NULL route_from,NULL route_to,NULL contract_amount,NULL pickup_amount,NULL step_amount,NULL warehouse_amount,NULL send_amount,"
