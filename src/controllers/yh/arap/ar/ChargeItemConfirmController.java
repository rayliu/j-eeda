...
public class ChargeItemConfirmController extends Controller {
	...
	public void list() {
		...
		sql = "SELECT "
				...
				+ " ror.remark,"
				+ " cast(ror.receipt_date as char) receipt_date,"
				...
				+ " dvr.ref_no ref_no,"
				+ "		  FORMAT((select sum(amount) from return_order_fin_item rofi where rofi.return_order_id= ror.id),2) total_amount,"
				+ "        ror.path,"
				+ "        ror.create_date,"
				+ "        null change_amount,"
				...
				+ "            IFNULL(cast(tor.planning_time as char), (SELECT "
				+ "                    GROUP_CONCAT(cast(tor.planning_time as char) SEPARATOR '<br/>')"
				+"                FROM"
				+"                    delivery_order dvr"
				+"                LEFT JOIN delivery_order_item doi ON doi.delivery_id = dvr.id"
				+"                LEFT JOIN transfer_order tor ON tor.id = doi.transfer_order_id"
				...
				+" 		      null serial_no,"
				+" 		      null ref_no,"
				+"            FORMAT(amco.total_amount, 2) total_amount,"
				+"            NULL path,"
