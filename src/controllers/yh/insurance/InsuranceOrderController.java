...
public class InsuranceOrderController extends Controller {
    ...
    public void createList() {
        ...
        if (orderNo == null &&  customer == null && routeFrom == null && routeTo == null
            ...
            sql = "select distinct tor.id,tor.order_no,tor.operation_type,tor.cargo_nature,tor.order_type,"
                    ...
                    + " tor.address,tor.pickup_mode,tor.arrival_mode,tor.status,c.abbr cname,c2.abbr spname,ifnull(ul.c_name,ul.user_name) create_by,tor.customer_order_no,"
    ...
    public void getInitPickupOrderItems() {
        ...
        String sql = "";
       	sql = "select distinct toi.id item_id,ifi.id fin_id,ifi.amount fin_amount,ifi.*,ifnull(toi.item_name, pd.item_name) item_name,ifnull(toi.item_no, pd.item_no) item_no,ifnull(toi.volume, pd.volume)*toi.amount volume, "
