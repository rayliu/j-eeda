...
public class StatusReportController extends Controller{
	...
	public void orderFlowList() {	
		...
		String sql = " SELECT CONCAT(cast(tor.id as char),':',tor.order_no, '-', tor. STATUS) transfer_order_no, "
				...
				+ "(select group_concat(cast(dor.id as char),':', dor.depart_no ,'-',dor.`STATUS` ,'/财务：',dor.`audit_status` ) "
				+ " from depart_order dor "
...
//                    + " round(ifnull(((sum((ys_insurance + return_amount)) - sum((yf_pickup + yf_depart + yf_insurance + delivery))) / sum((ys_insurance + return_amount))),0),2) profit_rate"
					...
					+ " dt.order_id = tor.id AND fi.type = '应付' AND pofi.pickup_order_id = toid.pickup_id "
					+ " AND pofi.fin_item_id != 7 AND d_o.audit_status = '对账已确认' AND d_o.combine_type = 'PICKUP') / ( SELECT count(0) FROM transfer_order_item_detail WHERE pickup_id = toid.pickup_id),0) yf_pickup,"
					...
					+ " WHERE dt.order_id = tor.id AND fi.type = '应付' AND dofi.depart_order_id = toid.depart_id"
					+ " AND d_o.audit_status = '对账已确认' AND d_o.combine_type = 'DEPART' ) / ( SELECT count(0) FROM transfer_order_item_detail WHERE depart_id = toid.depart_id ), 0) yf_depart,"
					...
					+ " ifnull("
					+ "        ("
					+ "           (SELECT IFNULL(sum(dofi1.amount), 0) FROM delivery_order d_o LEFT JOIN delivery_order_fin_item dofi1 ON dofi1.order_id = d_o.id "
					+ "             LEFT JOIN fin_item fi ON fi.id = dofi1.fin_item_id WHERE d_o.id = toid.delivery_id AND d_o.audit_status = '对账已确认' AND fi.type = '应付'"
					+ "           ) / (SELECT count(0) FROM transfer_order_item_detail WHERE delivery_id = toid.delivery_id)"
					+ "        ) + IFNULL((" //这里缺少ifnull 导致 delivery ==0
					+ "              ( SELECT IFNULL(sum(dofi1.amount), 0) FROM delivery_order d_o LEFT JOIN delivery_order_fin_item dofi1 ON dofi1.order_id = d_o.id LEFT JOIN fin_item fi ON fi.id = dofi1.fin_item_id"
					+ "                WHERE d_o.id = toid.delivery_refused_id AND d_o.audit_status = '对账已确认' AND fi.type = '应付' "
					+ "              ) / ( SELECT count(0) FROM transfer_order_item_detail WHERE delivery_refused_id = toid.delivery_refused_id)"
					+ "        ),0), 0) delivery,"
		...
		if (getPara("iDisplayStart") != null && getPara("iDisplayLength") != null) {
			sLimit = " LIMIT " + getPara("iDisplayStart") + ", " + getPara("iDisplayLength");
				...
				String yg_sql = " SELECT"
						...
						+ " 						delivery_order dor"
						+ " 					WHERE"
						+ " 						dor.id = toid.delivery_refused_id"
						+ " 				),"
						+ " 				''"
						+ " 			)"
						+ " 		) deliveryno,"
						+ " 		tor.order_no transferno,"
						+ " 		tor. STATUS,"
						+ " 		'' ORDER_TYPE,"
						+ " 		toid.serial_no,"
						+ " 		tor.planning_time,"
						...
						+ " 		l2. NAME route_to,"
						+ " 		toid.pieces,"
						+ " 		round(toid.weight, 2) weight,"
						+ " 		round(toid.volume, 2) volume,"
						+ " 		round("
						+ " 			ifnull("
						+ " 				("
						+ " 					SELECT"
						+ " 						IFNULL(sum(pofi.amount), 0)"
						+ " 					FROM"
						+ " 						pickup_order_fin_item pofi"
						...
						+ " 				("
						+ " 					SELECT"
						+ " 						IFNULL(sum(dofi.amount), (select cit.amount from contract_item cit "
						+ " LEFT JOIN contract ct on ct.id = cit.contract_id"
						+ " where "
						+ " ct.type = 'SERVICE_PROVIDER'"
						+ " and d_o.route_from = cit.from_id"
						+ " and d_o.route_to = cit.to_id"
						+ " and d_o.sp_id = ct.party_id"
						+ " and d_o.charge_type = cit.pricetype"
						+ " ))"
						+ " 					FROM"
						+ " 						depart_order_fin_item dofi"
						...
						+ " 						LEFT JOIN fin_item fi ON fi.id = dofi1.fin_item_id"
						+ " 						WHERE"
						+ " 							d_o.id = toid.delivery_id"
						+ " 						AND fi.type = '应付'"
						+ " 					) / ("
						+ " 						SELECT"
						+ " 							count(0)"
						...
						+ " 						LEFT JOIN fin_item fi ON fi.id = dofi1.fin_item_id "
						+ " 						WHERE "
						+ " 							d_o.id = toid.delivery_refused_id "
						+ " 						AND fi.type = '应付' "
						+ " 					) / ( "
						+ " 						SELECT "
						...
						+ " 							transfer_order_item_detail "
						+ " 						WHERE "
						+ " 							delivery_refused_id = toid.delivery_refused_id "
						+ " 					), "
						+ " 					0 "
						+ " 				), "
						+ " 				0 "
						...
						+ " 					sum(rof.amount) "
						+ " 				FROM "
						+ " 					return_order_fin_item rof "
						+ " 				LEFT JOIN return_order ror ON ror.id = rof.return_order_id "
						+ " 				LEFT JOIN delivery_order dor ON dor.id = ror.delivery_order_id "
						+ " 				WHERE "
						+ " 					dor.id = toid.delivery_id "
						+ " 				AND ( "
						+ " 					ror.transaction_status is not null "
						+ " 					AND ror.transaction_status != '手动删除' "
						+ " 				) "
						+ " 			), "
						+ " 			( "
						+ " 				SELECT "
						+ " 					ifnull(sum(rof1.amount), 0) "
						+ " 				FROM "
						+ " 					return_order_fin_item rof1 "
						+ " 				LEFT JOIN return_order ror ON ror.id = rof1.return_order_id "
						+ " 				WHERE "
						+ " 					ror.transfer_order_id = toid.order_id "
						+ " 				AND ( "
						+ " 					ror.transaction_status  is not null "
						+ " 					AND ror.transaction_status != '手动删除' "
						+ " 				) "
						...
						+ " 			( "
						+ " 				SELECT "
						+ " 					GROUP_CONCAT(ror.transaction_status) "
						+ " 				FROM "
						+ " 					return_order ror "
						+ " 				WHERE "
						+ " 					toid.delivery_id = ror.delivery_order_id "
						+ " 			), "
						+ " 			( "
						+ " 				SELECT "
						+ " 					GROUP_CONCAT(ror.transaction_status) "
						...
						+ " 		( "
						+ " 			SELECT "
						+ " 				GROUP_CONCAT(DISTINCT d_o.order_no  SEPARATOR '<br/>')"
						+ " 			FROM "
						+ " 				delivery_order d_o "
						...
						+ " 									depart_transfer dof "
						+ " 								WHERE "
						+ " 									dof.pickup_id = dor1.id "
						+ " 							) "
						+ " 						FROM "
						+ " 							pickup_order_fin_item pof "
						+ " 						LEFT JOIN depart_order dor1 ON dor1.id = pof.pickup_order_id "
						+ " 						WHERE "
						+ " 							dor1.id = dtr.pickup_id "
						+ " 						AND pof.fin_item_id != 7 "
						+ " 					) "
						+ " 				), "
						+ " 				0 "
						+ " 			), "
						...
						+ " 					( "
						+ " 						SELECT "
						+ " 							IFNULL(sum(amount), (select cit.amount from contract_item cit  "
						+ " LEFT JOIN contract ct on ct.id = cit.contract_id "
						+ " where  "
						+ " ct.type = 'SERVICE_PROVIDER' "
						+ " and dor1.route_from = cit.from_id "
						+ " and dor1.route_to = cit.to_id "
						+ " and dor1.sp_id = ct.party_id "
						+ " and dor1.charge_type = cit.pricetype "
						+ " and dor1.car_type = cit.carType"
						+ " )) / ( "
						+ " 								SELECT "
						+ " 									count(*) "
						+ " 								FROM "
						+ " 									depart_transfer dof "
						+ " 								WHERE "
						+ " 									dof.depart_id = dor1.id "
						+ " 							) "
						+ " 						FROM "
						+ " 							depart_order_fin_item dof "
						+ " 						LEFT JOIN depart_order dor1 ON dor1.id = dof.depart_order_id "
						+ " 						WHERE "
						+ " 							dor1.id = dtr.depart_id "
						+ " 						AND dof.fin_item_id != 7 "
						+ " 					) "
						+ " 				), "
						+ " 				0 "
						+ " 			), "
						...
						+ " 					FROM "
						+ " 						insurance_order i "
						+ " 					LEFT JOIN transfer_order t ON t.insurance_id = i.id "
						+ " 					WHERE "
						+ " 						i.id = ( "
						+ " 							SELECT "
						+ " 								id "
						...
						+ " 		( "
						+ " 			SELECT "
						+ " 				 GROUP_CONCAT(DISTINCT dor. STATUS  SEPARATOR '<br/>')"
						+ " 			FROM "
						+ " 				delivery_order dor "
						+ " 			LEFT JOIN delivery_order_item doi ON doi.delivery_id = dor.id "
						+ " 			WHERE "
				...
				String yy_sql = "SELECT *,round((yf_pickup+yf_depart+yf_insurance+delivery),2) yf_sum,round((ys_insurance+return_amount),2) ys_sum,round(((ys_insurance+return_amount)-(yf_pickup+yf_depart+yf_insurance+delivery)),2)yz_amount,round(ifnull((((ys_insurance+return_amount)-(yf_pickup+yf_depart+yf_insurance+delivery))/(ys_insurance+return_amount)),0),2) maolilv,"
		                ...
		                +" 		end"
		                +" 		) province, "
						+ " c.abbr,(SELECT	GROUP_CONCAT(DISTINCT d_o.order_no SEPARATOR '<br/>') FROM	delivery_order d_o LEFT JOIN delivery_order_item doi ON doi.delivery_id = d_o.id WHERE doi.transfer_order_id = tor.id) deliveryno,tor.order_no transferno,tor.STATUS,'' ORDER_TYPE,'' serial_no,tor.planning_time,l1. NAME route_from,l2. NAME route_to,"
