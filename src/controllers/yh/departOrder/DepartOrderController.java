...
public class DepartOrderController extends Controller {
    ...
    public void list() {
        ...
        String sqlTotal = "select count(1) total FROM DEPART_ORDER do " + " left join party p on do.driver_id = p.id  and p.party_type = '"
                + Party.PARTY_TYPE_DRIVER + "'" + " left join contact c on p.contact_id = c.id " + " where combine_type = '"
    ...
    public void createTransferOrderList() {
        ...
        String sqlTotal = "select count(1) total  from transfer_order to " + " left join party p on to.customer_id = p.id "
                + " left join contact c on p.contact_id = c.id " + " left join location l1 on to.route_from = l1.code "
        ...
        String sql = "select to.id,to.order_no,to.cargo_nature,"
                ...
                + " left join party p on to.customer_id = p.id " + " left join contact c on p.contact_id = c.id "
                + " left join location l1 on to.route_from = l1.code " + " left join location l2 on to.route_to = l2.code"
                + " where to.status not in ('已入库','已签收') and arrival_mode not in ('delivery')" + " order by to.CREATE_STAMP desc";
    ...
    public void getIintDepartOrderItems() {
        ...
        String sqlTotal = "select count(1) total from TRANSFER_ORDER_ITEM tof" + " left join TRANSFER_ORDER  or  on tof.ORDER_ID =or.id "
                + " left join CONTACT c on c.id in (select contact_id from party p where or.customer_id=p.id)" + " where tof.ORDER_ID in("
                + order_id + ")";
        Record rec = Db.findFirst(sqlTotal);
        logger.debug("total records:" + rec.getLong("total"));
    ...
    public void savedepartOrder() {
        ...
        if ("".equals(depart_id)) {
            ...
            dp = new DepartOrder();
            dp.set("CREATE_BY", Integer.parseInt(creat_id)).set("create_stamp", createDate).set("combine_type", "DEPART")
                    .set("car_no", getPara("car_no")).set("car_type", getPara("cartype")).set("depart_no", depart_no)
                    .set("DRIVER_ID", Integer.parseInt(party_id)).set("car_size", getPara("carsize")).set("remark", getPara("remark"));
            dp.save();
