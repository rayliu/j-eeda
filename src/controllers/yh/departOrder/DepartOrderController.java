        ...
        if("ATM".equals(cargo_nature)){
        	if("perUnit".equals(charge_type)){
        		conditionSql =  " select d.*,"
    					...
    					+ " (select ci.amount from depart_order dor left join contract c on c.party_id = dor.sp_id left join contract_item ci on ci.contract_id = c.id where dor.id = d.depart_order_id and dor.charge_type = ci.pricetype and c.type ='SERVICE_PROVIDER') price,"
