...
=======
	...
	public void onTripList() {
		...
		if (orderNo == null && departNo == null && status == null && sp == null
			...
			sql = "select deo.id,deo.depart_no ,deo.create_stamp ,deo.status as depart_status,ct.contact_person,ct.phone,c.car_no,c.cartype,c.length,(select tr.arrival_mode from transfer_order tr where tr.id in(select order_id from depart_transfer dt where dt.depart_id=deo.id) group by tr.arrival_mode) arrival_mode,(select group_concat(tr.order_no separator '\r\n') from transfer_order tr where tr.id in(select order_id from depart_transfer dt where dt.depart_id=deo.id ))  as transfer_order_no  from depart_order deo "
					...
					+ " left join contact ct on p.contact_id = ct.id  where  ifnull(deo.status,'') != 'aa'  and combine_type = '"
					+ DepartOrder.COMBINE_TYPE_DEPART
					+ "' and deo.status in('已发车','在途') order by deo.create_stamp desc";
		} else {
			if (beginTime == null || "".equals(beginTime)) {
				beginTime = "1-1-1";
			}
			...
			sqlTotal = "select count(distinct deo.id) total from depart_order deo "
					...
					+ departNo
					+ "%' and "
					+ "ifnull(c.company_name,'')  like '%"
					+ sp
					+ "%' and "
					+ "ifnull(tr.order_no,'') like '%"
					+ orderNo
					+ "%'"
					+ " and deo.create_stamp between '"
					+ beginTime
	...
	public void saveDepartOrder() {
		...
		if ("".equals(depart_id)) {
			...
			dp.set("charge_type", charge_type)
					...
					.set("depart_no", getPara("order_no"))
	...
	// priceType 不分大小写在mysql会有问题
	private void calcCost(List<UserLogin> users, DepartOrder departOrder,
		...
		Contract spContract = Contract.dao
				.findFirst("select * from contract where type='SERVICE_PROVIDER' "
		...
		if (spId != null) {
			if ("perUnit".equals(chargeType)) {
				genFinPerUnit(users, departOrder, transferOrderItemList,
						spContract, chargeType);
			} else if ("perCar".equals(chargeType)) {
				genFinPerCar(users, departOrder, spContract, chargeType);
			} else if ("perCargo".equals(chargeType)) {
	...
	private void genFinPerCar(List<UserLogin> users, DepartOrder departOrder,
			Contract spContract, String chargeType) {
		// 根据发车单整车的车型，长度，始发地，目的地，计算合同价
		Record contractFinItem = Db
				.findFirst("select amount, fin_item_id from contract_item where contract_id ="
						+ spContract.getLong("id")
						+ " and cartype = "
						+ departOrder.get("car_type")
						+ " and carlength = "
						+ departOrder.get("car_size")
						+ " and from_id = '"
	...
	private void genFinPerUnit(List<UserLogin> users, DepartOrder departOrder,
		...
		for (Record tOrderItemRecord : transferOrderItemList) {
			...
			Record contractFinItem = Db
					.findFirst("select amount, fin_item_id from contract_item where contract_id ="
							...
							+ "' and to_id = '"
							+ tOrderItemRecord.get("route_to")
							+ "' and priceType='" + chargeType + "'");
			if (contractFinItem != null) {
				genFinItem(users, departOrder, tOrderItemRecord,
						contractFinItem);
			} else {
				contractFinItem = Db
						.findFirst("select amount, fin_item_id from contract_item where contract_id ="
	...
	private void genFinItem(List<UserLogin> users, DepartOrder departOrder,
	...
	public static String creatOrderNo() {
		...
		ReturnOrder order = ReturnOrder.dao
				.findFirst("select * from return_order "
					...
					if (transferOrderItem != null) {
						if (transferOrderItem.get("product_id") != null) {
							...
							if (inventoryItem == null) {
								...
								inventoryItem.set(
										"total_quantity",
										Double.parseDouble(inventoryItem.get(
	...
	public void accountPayable() {
		...
		// 获取总条数
		String totalWhere = "";
		String sql = "select count(1) total from transfer_order_fin_item where depart_id = '"
				+ id + "'";
		Record rec = Db.findFirst(sql + totalWhere);
		logger.debug("total records:" + rec.getLong("total"));
		...
		List<Record> orders = Db
				.find("select d.*,f.name,f.remark from depart_order_fin_item d "
						+ "left join fin_item f on d.fin_item_id = f.id "
						+ "where d.depart_order_id ='"
						+ id
						+ "' and f.type='应付'");
	...
	public void addNewRow() {
		...
		fItem.set("type", "应付");
		fItem.save();
		dFinItem.set("fin_item_id", fItem.get("id")).set("status", "新建")
				.set("depart_order_id", pickupOrderId);
		dFinItem.save();
		renderJson("{\"success\":true}");
	}
