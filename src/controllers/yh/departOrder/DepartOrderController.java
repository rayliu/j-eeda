    ...
    public void createTransferOrderList() {
        ...
        if (orderNo == null && status == null && address == null && customer == null && routeFrom == null
            ...
            sql = "select distinct tor.id,tor.order_no,tor.planning_time,tor.operation_type,tor.cargo_nature, tor.arrival_mode ,"
                    ...
                    + " left join depart_order dd on dd.id = dt.pickup_id where dt.order_id = tor.id and (dd.status='已入货场' or dd.status='已入库') "
                    + " and exists ( select 1 from depart_pickup where order_id = tor.id and pickup_id = dt.pickup_id) ) pickup_id," 
                    ...
                    + " where dt.order_id = tor.id and (dor.status='已入货场' or dor.status='已入库') and "
                    + " exists (select 1 from depart_pickup where order_id = tor.id and pickup_id = dt.pickup_id) ) pickup_no,"
                    ...
                    + " (SELECT	l1.NAME FROM location l1 WHERE tor.route_from = l1.CODE) route_from,"
                    + " (SELECT l2.NAME FROM location l2 WHERE	tor.route_to = l2.CODE) route_to,"
                    + " tor.create_stamp ,cont.abbr as spname,cont.id as spid,"
                    + " (SELECT	o.office_name from office o	WHERE o.id = tor.office_id) office_name "
...
//                    + " left join office o on o.id=tor.office_id "
            ...
            sql = "select distinct tor.id,tor.order_no,tor.planning_time,tor.operation_type,tor.cargo_nature, tor.arrival_mode ,"
                    ...
                    + " and exists (select 1 from depart_pickup where order_id = tor.id and pickup_id = dt.pickup_id) ) pickup_id," 
                    + " (select group_concat( distinct dor.depart_no separator '\r\n' ) from depart_transfer dt left join depart_order dor on dor.id = dt.pickup_id left join depart_pickup dp on dp.pickup_id = dt.pickup_id "
                    + " where dt.order_id = tor.id and (dor.status='已入货场' or dor.status='已入库') "
                    + " and exists (select 1 from depart_pickup where order_id = tor.id and pickup_id = dt.pickup_id)  ) pickup_no,"
                    ...
                    + " tor.create_stamp,tor.depart_assign_status,c2.abbr spname, "
                    + " (SELECT	o.office_name from office o	WHERE o.id = tor.office_id) office_name "
                    + " from transfer_order tor "
                    + " left join party p on tor.customer_id = p.id "
                    + " left join contact c on p.contact_id = c.id "
    	...
    	if("arrangementOrder".equals(transferOrder.get("order_type")) || ("cargoReturnOrder".equals(transferOrder.get("order_type")) && "deliveryToFachtoryFromWarehouse".equals(transferOrder.get("arrival_mode")))){
    		...
    		for (TransferOrderItem transferOrderItem : list) {
    			if(transferOrderItem.getLong("product_id") != null && transferOrderItem.getLong("product_id") != 0 ){
        			...
        			TransferOrderItemDetail toid = TransferOrderItemDetail.dao.findFirst("select count(*) as amount from transfer_order_item_detail where item_id = ? and depart_id = ? and pickup_id is null ",transferOrderItem.get("id"),departOrderId);
