...
public class DataInitUtil {
    public static void initH2Tables(C3p0Plugin cp) {
        try {
            ...
            stmt.executeUpdate("create table if not exists transfer_order(id bigint auto_increment primary key,order_no varchar(255),status varchar(255),assign_status varchar(255),"
                    + "cargo_nature varchar(255),pickup_mode varchar(255),arrival_mode varchar(255),remark varchar(255),pickup_seq varchar(255),car_size varchar(255),car_no varchar(255),car_type varchar(255),create_by bigint,"
    ...
    public static void initData(C3p0Plugin cp) {
        try {
            ...
            subCat1.set("name", "ATM").set("customer_id", party.get("id"))
                    .set("parent_id", rootCat.getLong("id")).save();

            Category subCat2 = new Category();
            subCat2.set("name", "普通货品").set("customer_id", party.get("id"))
                    .set("parent_id", rootCat.getLong("id")).save();
            ...
            stmt.execute("insert into product(item_name,item_no,size,width,volume,weight,category_id,item_desc) values('普通货品', '2014042600002','2','4','5','7',3, '这是普通货品');");
            ...
            // 运输单
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,warehouse_id,route_from,route_to,office_id,order_type,customer_province,position) values('ATM', '4', '12', 'YS2014042600001', '3', 'routeSP', '1', '新建', '2014-04-20 16:33:35.1', 'delivery','珠海','2','110102','440402','2','salesOrder','provinceOut','1');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,warehouse_id,route_from,route_to,office_id,order_type,customer_province,position) values('cargo', '5', '13', 'YS2014042600002', '4', 'pickupSP', '2', '新建', '2014-04-16 16:40:35.1', 'gateIn','中山','2','110105','442000','3','salesOrder','provinceIn','2');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,route_from,route_to,office_id,order_type,customer_province,position) values('cargo', '6', '12', 'YS2014042600003', '4', 'pickupSP', '3', '新建', '2014-04-28 16:46:35.1', 'gateIn','广州','110106','440403','2','salesOrder','provinceOut','3');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,route_from,route_to,office_id,order_type,customer_province,position) values('cargo', '8', '13', 'YS2014042600004', '3', 'own', '1', '已发车', '2014-04-25 16:35:35.1', 'gateIn','深圳','110107','440404','1','salesOrder','provinceIn','4');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,route_from,route_to,office_id,order_type,customer_province,position) values('ATM', '7', '12', 'YS2014042600005', '3', 'own', '2', '已发车', '2014-04-22 16:28:35.1', 'delivery','东莞','110108','440507','1','arrangementOrder','provinceOut','5');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,route_from,route_to,warehouse_id,office_id,order_type,customer_province,position) values('ATM', '9', '13', 'YS2014042600006', '3', 'own', '3', '已发车', '2014-04-24 16:58:35.1', 'gateIn','东莞','110109','440511','2','2','arrangementOrder','provinceIn','6');");
            stmt.executeUpdate("insert into transfer_order(cargo_nature, sp_id, notify_party_id, order_no, create_by, pickup_mode, customer_id, status, create_stamp, arrival_mode,address,route_from,route_to,warehouse_id,office_id,order_type,customer_province,position) values('ATM', '10', '12', 'YS2014042600007', '3', 'own', '3', '已入库', '2014-04-24 16:58:35.1', 'gateIn','广州','','','1','3','arrangementOrder','provinceOut','7');");
            // 货品明细
            ...
            stmt.executeUpdate("insert into transfer_order_item(amount,order_id,product_id) "
                    + "values(2, 1, 1);");
            stmt.executeUpdate("insert into transfer_order_item(item_no, item_name, item_desc,amount,unit,volume,weight,remark,order_id) "
            ...
            stmt.execute("insert into depart_order(depart_no,create_stamp,combine_type,car_no,car_type,driver_id,car_size,status,create_by) values('PC2014061000002', '2014-06-10 10:39:35.1','PICKUP','粤A879588','箱式货车','27','25','新建',4);");
    ...
    public static void newCustomer() {
        ...
        contact2.set("company_name", "北京制药珠海分公司").set("contact_person", "黄生")
                .set("email", "test@test.com").set("abbr", "北京制药珠分");
