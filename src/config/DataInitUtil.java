    ...
    public static void initH2Tables(C3p0Plugin cp) {
        try {
            ...
            stmt.executeUpdate("create table if not exists arap_cost_order(id bigint auto_increment primary key,order_no varchar(255),order_type varchar(255),status varchar(255),amount double,payee_id varchar(255),create_by bigint,confirm_by bigint,begin_time timestamp,end_time timestamp,"
            		+ " create_stamp timestamp,confirm_stamp timestamp,last_modified_by bigint,last_modified_stamp timestamp,remark varchar(5120),total_amount double,application_order_id bigint,foreign key(application_order_id) references arap_cost_invoice_application_order(id));");
