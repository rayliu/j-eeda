...
public class DataInitUtil {
	...
	public static void initData(C3p0Plugin cp) {
		try {
			...
			stmt.executeUpdate("insert into return_order(status_code,create_date,transaction_status,order_type,creator,remark,transfer_order_id,notity_party_id,customer_id,route_id) values('20132015',CURRENT_TIMESTAMP(),'未完成','应付','张三','这是一张回单','2','2','2','2');");
			...
			stmt.executeUpdate("insert into Fin_item(code,name,type,Remark) values("
					+ "'20132014','干线运输费','应收','这是一张运输单收费');");
			stmt.executeUpdate("insert into Fin_item(code,name,type,Remark) values("
					+ "'20132015','搬运费','应收','这是一张运输单收费');");
			stmt.executeUpdate("insert into Fin_item(code,name,type,Remark) values("
					+ "'20132016','上楼费','应收','这是一张运输单收费');");
